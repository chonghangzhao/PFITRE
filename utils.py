...
def img_pad(img_torch, pad_mod='circular'):
    '''
    pad the size of square image to the times of 4
    ...
    '''
    pad_len = 4-(img_torch.size()[-1])%4

    if (pad_len%2 ==0)&(pad_len !=4):
        padding_dimen = (int(pad_len/2), int(pad_len/2), int(pad_len/2), int(pad_len/2))
    elif (pad_len%2 !=0)&(pad_len !=4):
        padding_dimen = (int(np.round(pad_len/2)),int(np.round(pad_len/2)-1), int(np.round(pad_len/2)),int(np.round(pad_len/2)-1))
    elif pad_len ==4:
        padding_dimen = (0,0,0,0)

    padded_image = F.pad(img_torch, padding_dimen, mode=pad_mod)
