...
def norm_inNout(img_in, Model, norm='self', pad_mod='constant', percentile=0.01):
    ...
    if len(img_in.shape) == 3:
        ...
        warnings.warn("Input format can only be 2d or 3d np array")
...
def recon_ADMM_NN_TV(sinogram, theta, Model, iter_num=8, ADMM_rho_const=5, cor_shift=0, initial=None, mask_boundary=False, mask_ratio=0.95, norm_quant=False, TV=True):
    """
    sinogram: 2darray
        input sinogram 
    ...
    TV:
        True: apply total variation regularization 
        False: without total variation regularization
    ...
    if pad_len%2 ==0:
        ...
        sinogram = np.pad(sinogram, ((0,0),(int(math.ceil(pad_len/2)),int(math.ceil(pad_len/2)-1))),mode='constant', constant_values=(0,0))  #, mode='minimum') 
    ...
    for iters in tqdm(range(0,iter_num)):
        ...
        if isinstance(norm_quant, float)==True:
            x_prox_reg2d, x_prox_reg_min, x_prox_reg_max = image2d_tomodel_range_quantile(x_prox_reg, percentile=norm_quant, togpu=False)
